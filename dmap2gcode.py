...
if NUMPY == True:
    try:
        try:
            import numpy as numarray #import numpy.numarray as numarray # Ca mache mieux sous Mac
            import numpy.core
...
class Application(Frame):
    def __init__(self, master):
        ...
        self.y = -1
        if PIL == False:
            fmessage(translate_("Python Imaging Library (PIL) was not found"))
            fmessage(translate_("    The program will function but some functionality will be disabled."))
            fmessage(translate_("    PIL enables more image file formats and enables the"))
            fmessage(translate_("    preview image to scale with the window size."))
        if NUMPY == False:
            fmessage(translate_("NumPy was not found."))
            fmessage(translate_("    The program will function properly but the speed will be reduced."))
            fmessage(translate_("    With NumPy the speed of gcode generation is about eight times faster."))
    ...
    def createWidgets(self):
        ...
        ##########################################################################
        self.gpre.set("G17 G90 M3 S3000") # G64 P0.001  pas possible avec GRBL
        ...
        self.statusMessage.set(translate_("Welcome to dmap2gcode"))
        ##########################################################################
        ...
        # Buttons
        self.Save_Button = Button(self.master,text=translate_("Save G-Code"),command=self.menu_File_Save_G_Code_File_Finish)
        self.Roughing_but = Button(self.master,text=translate_("Open Roughing Settings"),command=self.ROUGH_Settings_Window)

        # Canvas
        lbframe = Frame( self.master )
        self.PreviewCanvas_frame = lbframe
        self.PreviewCanvas = Canvas(lbframe, width=self.w-725, \
                                        height=self.h-200, background="grey")#525
        ...
        # Left Column #
        self.Label_font_prop = Label(self.master,text=translate_("Image Size:"), anchor=W)

        self.Label_Yscale = Label(self.master,text=translate_("Image Height"), anchor=CENTER)
        ...
        self.Label_Yscale2 = Label(self.master,text=translate_("Image Width"), anchor=CENTER)
        ...
        self.Label_PixSize = Label(self.master,text=translate_("Pixel Size"), anchor=CENTER)
        ...
        self.Label_pos_orient = Label(self.master,text=translate_("Image Properties:"),\
                                          anchor=W)
        ...
        #Radio Button
        self.Label_Invert_Color_FALSE = Label(self.master,text=translate_("Depth Color"))
        self.Radio_Invert_Color_FALSE = Radiobutton(self.master,text=translate_("Black"), value=False,
                                         width="100", anchor=W)
        self.Radio_Invert_Color_FALSE.configure(variable=self.invert )
        #self.Label_Invert_Color_TRUE = Label(self.master,text=" ")
        self.Radio_Invert_Color_TRUE = Radiobutton(self.master,text=translate_("White"), value=True,
                                         width="100", anchor=W)
        self.Radio_Invert_Color_TRUE.configure(variable=self.invert )


        self.Label_normalize = Label(self.master,text=translate_("Normalize Depth"))
        ...
        self.Label_CutTop = Label(self.master,text=translate_("Cut Top Surface"))
        ...
        self.Label_Toptol = Label(self.master,text=translate_("Top Tolerance"), anchor=CENTER )
        ...
        # Right Column #
        self.Label_tool_opt = Label(self.master,text=translate_("Tool Properties:"), anchor=W)

        self.Label_ToolDIA = Label(self.master,text=translate_("Tool DIA"))
        ...
        self.Label_Tool      = Label(self.master,text=translate_("Tool End"), anchor=CENTER )
        self.Tool_OptionMenu = OptionMenu(root, self.tool, "Ball","V","Flat",\
                                               command=self.Set_Input_States_Event)
                
        self.Label_Vangle = Label(self.master,text=translate_("V-Bit Angle"), anchor=CENTER )
        ...
        self.Label_gcode_opt = Label(self.master,text=translate_("Gcode Properties:"), anchor=W)

        self.Label_Scanpat      = Label(self.master,text=translate_("Scan Pattern"), anchor=CENTER )
        self.ScanPat_OptionMenu = OptionMenu(root, self.scanpat, "Rows","Columns",\
                                            "R then C", "C then R")

        self.Label_CutPerim = Label(self.master,text=translate_("Cut Perimeter"))
        self.Checkbutton_CutPerim = Checkbutton(self.master,text=" ", \
                                              anchor=W, command=self.Set_Input_States)
        self.Checkbutton_CutPerim.configure(variable=self.cutperim)

        self.Label_Scandir      = Label(self.master,text=translate_("Scan Direction"), anchor=CENTER )
        self.ScanDir_OptionMenu = OptionMenu(root, self.scandir, "Alternating", "Positive",
                                            "Negative", "Up Mill", "Down Mill")
        
        self.Label_Feed = Label(self.master,text=translate_("Feed Rate"))
        ...
        self.Label_p_feed = Label(self.master,text=translate_("Plunge Feed"), anchor=CENTER )
        ...
        self.Label_StepOver = Label(self.master,text=translate_("Stepover"), anchor=CENTER )
        ...
        self.Label_Zsafe = Label(self.master,text=translate_("Z Safe"))
        ...
        self.Label_Zcut = Label(self.master,text=translate_("Max Cut Depth"))
        ...
        top_File.add("command", label = translate_("Open G-Code File"), \
                         command = self.menu_File_Open_G_Code_File)

        top_File.add("command", label = translate_("Open Image File"), \
                             command = self.menu_File_Open_IMAGE_File)

        top_File.add("command", label = translate_("Save G-Code File"), \
                         command = self.menu_File_Save_G_Code_File_Finish)
        top_File.add("command", label = translate_("Save Roughing G-Code File"), \
                         command = self.menu_File_Save_G_Code_File_Rough)
        if IN_AXIS:
            top_File.add("command", label = translate_("Write To Axis and Exit"), \
                             command = self.WriteToAxis)
        else:
            top_File.add("command", label = translate_("Exit"), command = self.menu_File_Quit)
        self.menuBar.add("cascade", label=translate_("File"), menu=top_File)

        top_Edit = Menu(self.menuBar, tearoff=0)
        top_Edit.add("command", label = translate_("Copy G-Code Data to Clipboard"), \
                         command = self.CopyClipboard_GCode)
        self.menuBar.add("cascade", label=translate_("Edit"), menu=top_Edit)

        top_View = Menu(self.menuBar, tearoff=0)
        top_View.add("command", label = translate_("Refresh"), command = self.menu_View_Refresh)

        top_View.add_separator()

        top_View.add_checkbutton(label = translate_("Show Origin Axis"),  variable=self.show_axis , \
                                     command= self.menu_View_Refresh)

        self.menuBar.add("cascade", label=translate_("View"), menu=top_View)

        top_Settings = Menu(self.menuBar, tearoff=0)
        top_Settings.add("command", label = translate_("General Settings"), \
                             command = self.GEN_Settings_Window)
        top_Settings.add("command", label = translate_("Roughing Settings"), \
                             command = self.ROUGH_Settings_Window)

        self.menuBar.add("cascade", label=translate_("Settings"), menu=top_Settings)

        top_Help = Menu(self.menuBar, tearoff=0)
        top_Help.add("command", label = translate_("About"), command = self.menu_Help_About)
        top_Help.add("command", label = translate_("Help (Web Page)"), command = self.menu_Help_Web)
        self.menuBar.add("cascade", label=translate_("Help"), menu=top_Help)
        ...
        try:
            ...
            fmessage(translate_('Unable interpret command line options'))
            sys.exit()
        for option, value in opts:
            if option in ('-h','--help'):
                fmessage(' ')
                fmessage(translate_('Usage: python dmap2gcode.py [-g file]'))
                fmessage(translate_('-g    : dmap2gcode gcode output file to read (also --gcode_file)'))
                fmessage(translate_('-h    : print this help (also --help)\n'))
                sys.exit()
    ...
    def entry_set(self, val2, calc_flag=0, new=0):
        if calc_flag == 0 and new==0:
            try:
                ...
                self.statusMessage.set(translate_(" Recalculation required."))
            except:
                pass
        elif calc_flag == 3:
            try:
                ...
                self.statusMessage.set(translate_(" Value should be a number. "))
            except:
                pass
        elif calc_flag == 2:
            try:
...
################################################################################
    def Write_Config_File(self, event):
        ...
        if ( os.path.isfile(configname_full) ):
            if not message_ask_ok_cancel(translate_("Replace"), translate_("Replace Exiting Configuration File?\n") +configname_full):
                try:
                    win_id.withdraw()
                    win_id.deiconify()
                except:
        ...
        try:
            ...
            self.statusMessage.set(translate_("Unable to open file for writing: %s") %(configname_full))
            self.statusbar.configure( bg = 'red' )
            return
        ...
        fout.close
        self.statusMessage.set(translate_("Configuration File Saved: %s") %(configname_full))
        self.statusbar.configure( bg = 'white' )
    ...
    def WriteGCode(self,rough_flag = 0,config_file=False):
        ...
        try:
            ...
            try:
                ...
                self.statusMessage.set(translate_("No Image Loaded"))
                self.statusbar.configure( bg = 'red' )
                return
        ...
        if converter == "Positive":
            ...
            conv_index = 2
            fmessage(translate_("Converter Error: Setting to, Alternating"))
    ...
    def CopyClipboard_GCode(self):
        ...
        self.statusMessage.set(translate_("G-Code Sent to Clipboard"))
    ...
    def Quit_Click(self, event):
        self.statusMessage.set(translate_("Exiting!"))
        root.destroy()
    ...
    def Entry_Yscale_Check(self):
        try:
            ...
            if  value <= 0.0:
                self.statusMessage.set(translate_(" Height should be greater than 0 "))
    ...
    def Entry_Toptol_Check(self):
        try:
            ...
            if  value > 0.0:
                self.statusMessage.set(translate_(" Tolerance should be less than or equal to 0 "))
                return 2 # Value is invalid number
        except:
            return 3     # Value not a number
        return 0         # Value is a valid number
    ...
    def Entry_ToolDIA_Check(self):
        try:
            ...
            if  value <= 0.0:
                self.statusMessage.set(translate_(" Diameter should be greater than 0 "))
                return 2 # Value is invalid number
        except:
            return 3     # Value not a number
        return 0         # Value is a valid number
    ...
    def Entry_Vangle_Check(self):
        try:
            ...
            if  value <= 0 or value >= 180:
                self.statusMessage.set(translate_(" Angle should be between 0 and 180"))
                return 2 # Value is invalid number
        except:
            return 3     # Value not a number
        return 0         # Value is a valid number
    ...
    def Entry_Feed_Check(self):
        try:
            ...
            if  value <= 0.0:
                self.statusMessage.set(translate_(" Feed should be greater than 0 "))
                return 2 # Value is invalid number
        except:
            return 3     # Value not a number
    ...
    def Entry_StepOver_Check(self):
        try:
            ...
            if  value <= 0.0:
                self.statusMessage.set(translate_(" Stepover should be greater than 0 "))
                return 2 # Value is invalid number
        except:
            return 3     # Value not a number
        return 0         # Value is a valid number
    ...
    def Entry_Zsafe_Check(self):
        try:
            ...
            if  value <= 0.0:
                self.statusMessage.set(translate_(" Z safe should be greater than 0 "))
                return 2 # Value is invalid number
        except:
            return 3     # Value not a number
        return 1         # Value is a valid number changes do not require recalc
    ...
    def Entry_Zcut_Check(self):
        try:
            ...
            if  value >= 0.0:
                self.statusMessage.set(translate_(" Max depth should be less than 0 "))
                return 2 # Value is invalid number
        except:
            return 3     # Value not a number
        return 1         # Value is a valid number changes do not require recalc
    ...
    def ROUGH_Entry_Roffset_Check(self):
        try:
            ...
            if  value < 0.0:
                self.statusMessage.set(translate_(" Roughing offset should be greater than or equal to 0 "))
                return 2 # Value is invalid number
        except:
            return 3     # Value not a number
        return 0         # Value is a valid number
    ...
    def ROUGH_Entry_Rdepth_Check(self):
        try:
            ...
            if  value < 0.0:
                self.statusMessage.set(translate_(" Roughing depth per pass should be greater than or equal to 0 "))
                return 2 # Value is invalid number
        except:
            return 3     # Value not a number
        return 0         # Value is a valid number
    ...
    def Entry_Tolerance_Check(self):
        try:
            ...
            if  value <= 0.0:
                self.statusMessage.set(translate_(" Tolerance should be greater than 0 "))
                return 2 # Value is invalid number
        except:
            return 3     # Value not a number
        return 0         # Value is a valid number
    ...
    def Entry_ContAngle_Check(self):
        try:
            ...
            if  value <= 0.0 or value >= 90:
                self.statusMessage.set(translate_(" Contact angle should be between 0 and 90 "))
                return 2 # Value is invalid number
        except:
            return 3     # Value not a number
        return 0         # Value is a valid number
    ...
    def Check_All_Variables(self):
        ...
        if (GEN_error_cnt > 0):
            self.statusMessage.set(\
                translate_(" Entry Error Detected: Check Entry Values in General Settings Window "))
        if (MAIN_error_cnt > 0):
            self.statusMessage.set(\
                translate_(" Entry Error Detected: Check Entry Values in Main Window "))
        if (ROUGH_error_cnt > 0):
            self.statusMessage.set(\
                translate_(" Entry Error Detected: Check Entry Values in Roughing Settigns Window "))
    ...
    def Open_G_Code_File(self,filename):
        try:
            ...
            fmessage(translate_("Unable to open file: %s") %(filename))
            return
        ...
        if init_file != "None":
            if ( os.path.isfile(self.IMAGE_FILE) ):
                ...
                self.statusMessage.set(translate_("Image file not found: %s ") %(self.IMAGE_FILE))
    ...
    def Read_image_file(self,fileselect):
        ...
        if not ( os.path.isfile(fileselect) ):
            self.statusMessage.set(translate_("Image file not found: %s") %(fileselect))
            self.statusbar.configure( bg = 'red' )            
        else:
            self.statusMessage.set(translate_("Image file: %s ") %(fileselect))
            self.statusbar.configure( bg = 'white' ) 
            try:
                if PIL:
                ...
                self.statusMessage.set(translate_("Unable to Open Image file: %s") %(self.IMAGE_FILE))
                self.statusbar.configure( bg = 'red' )    
    ...
    def menu_File_Save_G_Code_File(self,rough_flag = 0):
        ...
        if filename != '' and filename != ():
            ...
            try:
                ...
                self.statusMessage.set(translate_("Unable to open file for writing: %s") %(filename))
                self.statusbar.configure( bg = 'red' )
                return

            vcalc_status = Toplevel(width=725, height=50) # 525
            ...
            self.statusMessage.set(translate_("Preparing Image Data"))
            self.statusbar.configure( bg = 'yellow' )

            STOP_CALC = 0
            self.stop_button = Button(vcalc_status,text=translate_("Stop Calculation"))
            ...
            vcalc_status.title(translate_('Saving File'))
            ...
            try: #Attempt to create temporary icon bitmap file
                ...
                fmessage(translate_("Unable to create temporary icon file."))
            ...
            for line in self.gcode:
                try:
                    ...
                    fmessage(translate_("skipping g-code line:") + line + translate_("; may be due to non ASCII character."));
                    pass
            fout.close
            if STOP_CALC == 0:
                self.statusMessage.set(translate_("File Saved: %s") %(filename))
                self.statusbar.configure( bg = 'white' )
            else:
                self.statusMessage.set(translate_("File Save Terminated"))
                self.statusbar.configure( bg = 'yellow' )
    ...
    def Master_Configure(self, event, update=0):
        ...
        if abs(self.w-w)>10 or abs(self.h-h)>10 or update==1:
            ...
            if 0 == 0:                
                ...
                self.PreviewCanvas.configure( width = self.w-555, height = self.h-50 ) #455
                self.PreviewCanvas_frame.place(x=260, y=10) #220
    ...
    def GEN_Settings_Window(self):
        ...
        #Radio Button
        D_Yloc=D_Yloc+D_dY
        self.Label_Units = Label(gen_settings,text=translate_("Units"))
        ...
        D_Yloc=D_Yloc+D_dY
        self.Label_Tolerance = Label(gen_settings,text=translate_("tolerance"))
        ...
        D_Yloc=D_Yloc+D_dY
        self.Label_Gpre = Label(gen_settings,text=translate_("G Code Header"))
        ...
        D_Yloc=D_Yloc+D_dY
        self.Label_Gpost = Label(gen_settings,text=translate_("G Code Postscript"))
        ...
        D_Yloc=D_Yloc+D_dY
        self.Label_LaceBound = Label(gen_settings,text=translate_("Lace Bounding"))
        ...
        D_Yloc=D_Yloc+D_dY
        self.Label_ContAngle = Label(gen_settings,text=translate_("LB Contact Angle"))
        ...
        #Radio Button
        D_Yloc=D_Yloc+D_dY
        self.Label_SplitStep = Label(gen_settings,text=translate_("Offset Stepover"))
        self.Label_SplitStep.place(x=xd_label_L, y=D_Yloc, width=113, height=21)


        self.Radio_SplitStep_N = Radiobutton(gen_settings,text=translate_("None"), value="0",
                                         width="100", anchor=W)
        ...
        self.Radio_SplitStep_H = Radiobutton(gen_settings,text=translate_("1/2 Step"), value="0.5",
                                         width="100", anchor=W)
        ...
        self.Radio_SplitStep_Q = Radiobutton(gen_settings,text=translate_("1/4 Step"), value="0.25",
                                         width="100", anchor=W)
        ...
        #Radio Button
        D_Yloc=D_Yloc+D_dY
        self.Label_PlungeType = Label(gen_settings,text=translate_("Plunge Type"))
        self.Label_PlungeType.place(x=xd_label_L, y=D_Yloc, width=113, height=21)
        self.Radio_PlungeType_S = Radiobutton(gen_settings,text=translate_("Vertical"), value="simple",
                                         width="100", anchor=W)
        ...
        self.Radio_PlungeType_A = Radiobutton(gen_settings,text=translate_("Arc"), value="arc",
                                         width="100", anchor=W)
        ...
        D_Yloc=D_Yloc+D_dY
        self.Label_Disable_Arcs = Label(gen_settings,text=translate_("Disable G-Code Arcs"))
        ...
        D_Yloc=D_Yloc+D_dY
        self.Label_no_com = Label(gen_settings,text=translate_("Suppress Comments"))
        ...
        self.Label_SaveConfig = Label(gen_settings,text=translate_("Configuration File"))
        self.Label_SaveConfig.place(x=xd_label_L, y=D_Yloc, width=113, height=21)

        self.GEN_SaveConfig = Button(gen_settings,text=translate_("Save"))
        ...
        self.GEN_Close = Button(gen_settings,text=translate_("Close"),command=self.Close_Current_Window_Click)
    ...
    def ROUGH_Settings_Window(self):
        ...
        rough_settings.title(translate_('Roughing Settings'))
        ...
        # Start Right Column
        D_Yloc=6
        self.ROUGH_Label_tool_opt = Label(rough_settings,text=translate_("Roughing Tool Properties:"), anchor=W)
        
        self.ROUGH_Label_Tool      = Label(rough_settings,text=translate_("Roughing Tool End"), anchor=CENTER )
        ...
        D_Yloc=D_Yloc+24
        self.ROUGH_Label_ToolDIA = Label(rough_settings,text=translate_("Roughing Tool DIA"))
        ...
        D_Yloc=D_Yloc+24
        self.ROUGH_Label_Vangle = Label(rough_settings,text=translate_("Roughing V-Bit Angle"), anchor=CENTER )
        ...
        D_Yloc=D_Yloc+6        
        self.ROUGH_Label_gcode_opt = Label(rough_settings,text=translate_("Roughing Gcode Properties:"), anchor=W)
        self.ROUGH_Label_gcode_opt.place(x=xd_label_L, y=D_Yloc, width=w_label*2, height=21)

        D_Yloc=D_Yloc+24
        self.ROUGH_Label_Scanpat      = Label(rough_settings,text=translate_("Roughing Scan Pattern"), anchor=CENTER )
        ...
        D_Yloc=D_Yloc+24
        self.ROUGH_Label_Scandir      = Label(rough_settings,text=translate_("Roughing Scan Direction"), anchor=CENTER )
        ...
        D_Yloc=D_Yloc+24
        self.ROUGH_Label_Feed = Label(rough_settings,text=translate_("Roughing Feed Rate"))
        ...
        D_Yloc=D_Yloc+24
        self.ROUGH_Label_p_feed = Label(rough_settings,text=translate_("Roughing Plunge Feed"), anchor=CENTER )
        ...
        D_Yloc=D_Yloc+24
        self.ROUGH_Label_StepOver = Label(rough_settings,text=translate_("Roughing Stepover"), anchor=CENTER )
        ...
        D_Yloc=D_Yloc+6
        self.ROUGH_Label_roughing_props = Label(rough_settings,text=translate_("Roughing Properties:"),anchor=W)        
        self.ROUGH_Label_roughing_props.place(x=xd_label_L, y=D_Yloc, width=w_label*2, height=21)

        D_Yloc=D_Yloc+24
        self.ROUGH_Label_Roffset = Label(rough_settings,text=translate_("Roughing Offset"), anchor=CENTER )
        ...
        D_Yloc=D_Yloc+24
        self.ROUGH_Label_Rdepth = Label(rough_settings,text=translate_("Roughing Depth/Pass"), anchor=CENTER )
        ...
        self.ROUGH_Save = Button(rough_settings,text=translate_("Save\nRoughing G-Code"),\
                                 command=self.menu_File_Save_G_Code_File_Rough)
        self.ROUGH_Save.place(x=Xbut, y=Ybut, width=130, height=40, anchor="e")
        
        self.ROUGH_Close = Button(rough_settings,text=translate_("Close"),\
...
def progress(a, b, START_TIME, GUI=[]):
    if IN_AXIS:
        ...
        message = translate_('%.1f %% ( %.1f Minutes Remaining | %.1f Minutes Total )') %( CUR_PCT, MIN_REMAIN, MIN_TOTAL )
        try:   
...
app.master.title(translate_("dmap2gcode V")+version)
